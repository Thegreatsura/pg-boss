...
| `invalid_schedule` | A stored schedule could not be evaluated (for example an unusable `timezone` written by an older release, or a recurrence rule on an instance whose release predates rule support) and was skipped for this cron pass; the remaining schedules are unaffected. Emitted once per broken schedule rather than on every pass, and again if the schedule is edited or the instance restarts | `queue`, `key`, `cron`, `timezone` |
| `missed_occurrences_capped` | A schedule with [`missed: 'all'`](./scheduling.md#catch-up-after-an-outage) came due more times while no cron pass ran than one pass sends for it. The most recent 1000 occurrences were sent and the rest dropped. Emitted once per pass that truncates, so a deployment resuming after a long outage reports it on the pass that catches up and not again | `queue`, `key`, `cron`, `timezone`, `limit`, `since` |
